...
from ui.manager_profile_form import Ui_manager_profile_form
from manage_playlist import ManagePlayWidget
from manage_user import ManageUserWidget
from change_pass import Change_pass
from PyQt5.QtWidgets import QFileDialog

class ManagerProfileWidget(QtWidgets.QWidget, Ui_manager_profile_form):
    def __init__(self, username, sql_connector, central_widget, parent=None):
        super(ManagerProfileWidget, self).__init__(parent)
        self.setupUi(self)
        self.parent = parent
    ...
    def setup(self):
        ...
        self.manage_user_button.clicked.connect(self.manage_user)
        ...
        cursor.execute(
            "SELECT title FROM music, user_music_like WHERE music.id = user_music_like.music_id and user_music_like.username=%s",
            (self.username,))
        for row in cursor:
            self.liked_mu_view_2.addItem(row[0])

        cursor.execute(
            "SELECT title FROM music, user_listen WHERE music.id = user_listen.music_id and user_listen.username=%s",
            (self.username,))
        for row in cursor:
        ...
        cursor.execute(
            "SELECT title FROM composer_music, music WHERE composer_music.music_id=music.id and username = %s",
            (self.username,))
        for row in cursor:
    ...
    def manage_user(self):
        self.hide()
        mana_user = ManageUserWidget(self.username, self.sql_connector, self.central_widget, self)
        self.central_widget.addWidget(mana_user)
        self.central_widget.setCurrentWidget(mana_user)
    ...
    def add_tag(self):
        cursor = self.sql_connector.cursor()
        cursor.execute("INSERT INTO user_tag VALUES(%s, %s)", (self.username, self.tags_combo_2.currentText()))
        self.sql_connector.commit()
        self.setup()
